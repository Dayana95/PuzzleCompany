	...
	Croppic.prototype = {
						...
						reader.onload = function (e) {
							...
							image.onload = function(){
								...
								if (that.options.onAfterImgUpload) {
									console.log("REQUESTED WIDTH: ", that.options.cropData['requestedWidth']);
									var width = that.options.cropData['requestedWidth'];
									var height = that.options.cropData['requestedHeight'];
									var filename = that.options.cropData['filename'];
									if (that.options.cropData['requestedWidth'] > that.imgInitW) {
										alert("Image is not wide enough");
										window.location.href = "/index.php?productname=" + filename + "&w=" + width + "&h=" + height;
									} else if (that.options.cropData['requestedHeight'] > that.imgInitH) {
										alert("Image is not tall enough");
										window.location.href = "/index.php?productname=" + filename + "&w=" + width + "&h=" + height;
									}
		...
		crop:function(){
			...
			var cropData = {
					...
					console.log("Original Info W: ", cropData.imgInitW);
					console.log("Original Info H: ", cropData.imgInitH);

					console.log("Info H: ", that.imgH);
					console.log("Original Info W: ", that.imgW);

					console.log("New info H: ", that.objH);
					console.log("New info W: ", that.objW);

					console.log("New info W: ", that.objW);

					console.log("Position info Y: ", Math.abs( parseInt( that.img.css('top') ) ));
					console.log("Position info X: ", Math.abs( parseInt( that.img.css('left') ) ));

			if (cropData.imgInitW < that.objW || cropData.imgInitH< that.objH) {
				that.options.onError.call(that,"tamaño incorrecto");
			}
			var formData;
			...
			if(typeof FormData == 'undefined'){
				...
				console.log("FormData: ", that.options.cropUrl);
				...
				for (var key in that.options.cropData) {
					if( that.options.cropData.hasOwnProperty(key) ) {
						console.log("KEY: " + key + " - " + that.options.cropData[key]);
